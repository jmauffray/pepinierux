...
<table width="760" border="0" class="page" align="center">
  ...
  <tr>
	...
	$sql_nom = "SELECT  nom, nom2, type FROM " . $tblpref ."client WHERE num_client = $client";
	...
	<form name='formu2' method='post' action='bon_suite.php'>
	  <table class="boiteaction">
	    ...
	    <tr>
	      <td class="texte0"><?php echo "$lang_article N°";
		include_once("include/configav.php");
		if ($use_categorie !='y') { 
		  

		  $rqSql = "SELECT uni, num, article, prix_htva, prix_htva_part FROM " . $tblpref ."article WHERE actif != 'non' ORDER BY article, prix_htva, prix_htva_part";
		  $result = mysql_query( $rqSql )
              or die( "ExÃÂ©cution requÃÂªte impossible.");?>
		...
		<?php
		  while ( $row = mysql_fetch_array( $result)) {
		    ...
		    $uni = $row["uni"];
    		?>
		<OPTION VALUE='<?php echo $num; ?>'><?php echo "$article $prix"; ?></OPTION>
		<?php
		  }
		?>
	      </SELECT>
	    ...
	    <tr>
	      <td class="texte0"><?php echo $lang_prix_remise; ?> </td>
	      <td class="texte0" colspan="3"><input name='prix_remise' type='text' id='prix_remise' size='6'></td>
	    </tr>
	    <tr>
	      <td class="texte0"><?php echo $lang_volume_pot; ?> </td>
	      <td class="texte0" colspan="3"><input name='volume_pot' type='text' id='volume_pot' size='6'></td>
	    </tr>
	      <tr>
