...
list($dateJour, $dateMois,$dateAnnee) = preg_split('/\//', $date, 3);
...
$sql9 = "SELECT date, quanti, remise, article, tot_art_htva, to_tva_art, taux_tva, uni, num_bon 
FROM " . $tblpref ."client 
...
echo "<tr><th>Quanti <th>$lang_article<th>$lang_remise<th>$lang_prix_h_tva<th>$lang_taux_tva<th>$lang_tot_tva<th>$lang_num_bon<th>$lang_date_bon</tr>";
while($data = mysql_fetch_array($req))
    {
		$quanti = $data['quanti'];
		$remise = $data['remise'];
		...
		$aDate = $data['date'];
		echo "<tr><td>$quanti<td>$article<td>$remise<td>$tot_htva<td>$taux<td>$tot_tva<td>$num_bon<td>$aDate</tr>";
		}
...
$sql1 = "INSERT INTO " . $tblpref ."facture(acompte, date_depart, date_echeance, coment, client, date_fact, total_fact_h, total_fact_ttc, list_num)
	 VALUES ('$acompte','$date_depart','$date_echeance', '$coment', '$client', '$dateAnnee-$dateMois-$dateJour', '$total_htva', '$total_ttc', '$list_num')";
